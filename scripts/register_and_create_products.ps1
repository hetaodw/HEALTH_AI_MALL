# Register a new merchant and create products

# 1. Register new merchant
$merchantBody = @{
    username = "newmerchant1"
    password = "Test123456"
    email = "newmerchant1@example.com"
    phone = "13900139001"
    role = "MERCHANT"
} | ConvertTo-Json

try {
    $registerResponse = Invoke-RestMethod -Uri "http://localhost:8080/v1/auth/register" -Method Post -Body $merchantBody -ContentType "application/json"
    Write-Host "Registration successful!" -ForegroundColor Green
    Write-Host "Code: $($registerResponse.code)" -ForegroundColor Cyan
    Write-Host "Msg: $($registerResponse.msg)" -ForegroundColor Cyan
} catch {
...
try {
    ...
    Write-Host "Code: $($loginResponse.code)" -ForegroundColor Cyan
    Write-Host "Msg: $($loginResponse.msg)" -ForegroundColor Cyan
