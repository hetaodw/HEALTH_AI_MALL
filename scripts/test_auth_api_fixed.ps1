...
Write-Host "Authentication API Tests (Fixed)" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan
Write-Host ""
...
try {
    ...
    if ($response.code -eq 200) {
        ...
        Write-Host "FAIL: User registration failed with code $($response.code): $($response.msg)" -ForegroundColor Red
        $testResults += [PSCustomObject]@{
            TestCase = "User Registration"
            Endpoint = "POST /v1/auth/register"
            Status = "FAIL"
...
try {
    ...
    if ($response.code -eq 400) {
        ...
        Write-Host "FAIL: Duplicate registration should have failed but succeeded with code $($response.code)" -ForegroundColor Red
        $testResults += [PSCustomObject]@{
            TestCase = "Duplicate Registration"
            Endpoint = "POST /v1/auth/register"
...
try {
    ...
    if ($response.code -eq 200) {
        ...
        Write-Host "FAIL: User login failed with code $($response.code): $($response.msg)" -ForegroundColor Red
        $testResults += [PSCustomObject]@{
            TestCase = "User Login"
            Endpoint = "POST /v1/auth/login"
            Status = "FAIL"
...
try {
    ...
    if ($response.code -eq 401) {
        ...
        Write-Host "FAIL: Wrong password login should have failed but succeeded with code $($response.code)" -ForegroundColor Red
        $testResults += [PSCustomObject]@{
            TestCase = "Wrong Password Login"
            Endpoint = "POST /v1/auth/login"
            Status = "FAIL (expected to fail but succeeded)"
...
try {
    ...
    if ($response.code -eq 200) {
        ...
        Write-Host "FAIL: User logout failed with code $($response.code): $($response.msg)" -ForegroundColor Red
        $testResults += [PSCustomObject]@{
            TestCase = "User Logout"
            Endpoint = "POST /v1/auth/logout"
            Status = "FAIL"
...
try {
    ...
    if ($response.code -eq 200) {
        ...
        Write-Host "FAIL: Existing account login failed with code $($response.code): $($response.msg)" -ForegroundColor Red
        $testResults += [PSCustomObject]@{
            TestCase = "Existing Account Login"
            Endpoint = "POST /v1/auth/login"
            Status = "FAIL"
...
try {
    ...
    if ($response.code -eq 200) {
        ...
        Write-Host "FAIL: Merchant account login failed with code $($response.code): $($response.msg)" -ForegroundColor Red
        $testResults += [PSCustomObject]@{
            TestCase = "Merchant Account Login"
            Endpoint = "POST /v1/auth/login"
            Status = "FAIL"
...
$testResults | Export-Csv -Path "scripts\test_auth_api_fixed_results.csv" -NoTypeInformation -Encoding UTF8
Write-Host "Test results saved to: scripts\test_auth_api_fixed_results.csv" -ForegroundColor Gray
